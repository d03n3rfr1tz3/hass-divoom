...
class Backpack(Divoom):
    ...
    def show_signal(self, number, color1=None, color2=None):
        """Show the traffic signal on the Divoom Backpack"""
        return Divoom.show_visualization(self, number, color1, color2) # same view, different meaning

    def show_visualization(self, number, color1=None, color2=None):
        self.unsupported("visualizations, it has no microphone")
